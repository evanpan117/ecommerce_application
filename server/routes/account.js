...
const getAccountByAccountId = async function (account_id) {
  try {
    const result = await pool.query(
      'select * from account a inner join admin_user b on a.userid = b.userid where a.account_id=$1',
      [account_id]
    );
...
async function getAccountByEmail(email) {
  try {
    const result = await pool.query(
      'select * from account a inner join admin_user b on a.userid = b.userid where b.email=$1',
      [email]
    );
